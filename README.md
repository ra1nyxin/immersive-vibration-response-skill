...
flowchart TD
    ...
    S --> I{首次使用或需要恢复连接？}
    I -->|是| HC[发送 PING 与 STATUS 健康检查]
    I -->|否| ACT{选择触觉反馈动作}
    HC --> C[命令客户端]
    ACT -->|日常反馈与奖励| H[HIT damage]
    ACT -->|少数精确基准场景| T[SET level]
    ACT -->|玩家明确要求或立即结束| X[STOP]
    H --> C
    T --> C
    X --> C
    ...
    ER --> A
    V -->|是| Q{命令类型}
    Q -->|HIT：立即回执| R[返回 QUEUED HIT]
    R -->|AI 不等待震动结束，继续对话与任务| A
    Q -->|所有合法命令| W[后台串口工作队列]
    W -->|PING、STATUS、SCAN、SERVICES、SET、STOP 的结果| K[返回串口状态回复]
    K --> C
    K -->|健康检查结果| AV{通信桥与设备可用？}
    AV -->|是| ACT
    AV -->|否| FB[继续互动并在自然节点重试]
    FB --> A
    W -->|USB 串口| U[ESP32-S3 串口输入]
    U --> F{固件命令解析}
    F -->|HIT| M1[伤害值乘以 10\n累加并钳制到 0 至 100]
    F -->|SET| M2[直接设定 0 至 100]
    F -->|STOP| M3[立即归零]
    F -->|PING / STATUS / SCAN / SERVICES| SR[输出固件状态或诊断结果]
    SR --> K
    M1 --> Y[更新当前等级与保持计时器]
    M2 --> Y
    M3 --> BR{BLE 服务已就绪？}
    ...
    Z --> DEC[每 50 毫秒降低 1 级\n直到等级归零]
    DEC --> BR
    BOOT[ESP32-S3 启动] --> SCAN[扫描名称为 GK36 的设备]
    SCAN --> BR
    BR -->|否| RC[继续扫描或等待重连]
    RC --> BR
    BR -->|是| J[构造 GK36 BLE 震动与电刺激数据包]
    J --> N[BLE 服务 0x1000\n写特征 0x1001]
    N --> O[低功率震动设备]
    O -->|触觉反馈| P
```
